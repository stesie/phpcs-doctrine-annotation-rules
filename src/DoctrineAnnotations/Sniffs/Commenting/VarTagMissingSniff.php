...
class VarTagMissingSniff implements Sniff
{
    /**
    ...
    private function processDocblock(File $phpcsFile, $stackPtr)
    {
        ...
        if (null === $this->getVarTagContent($phpcsFile, $stackPtr)) {
            $error = 'There must be a @var tag on Doctrine mapped properties';
            $phpcsFile->addError($error, $stackPtr, 'VarTagMissing');
        }
    }
     ...
     * @return string|null
     */
    private function getVarTagContent(File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
     ...
     * @return bool
     */
    private function isDoctrineMappedProperty($annotations)
    {
